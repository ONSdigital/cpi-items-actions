...
manual <- FALSE
test_metafile <- "test_metadata.csv" # new metadata file from BA if manual
test_new_csv <- "test_newdata.csv" # new CPI data if manual
test_unchained <- "test_unchained.csv" # old unchained file if manual
...
if (update) {
  ...
  # 3 dp for perc growth and chaining
  addStyle(
    wb,
    "Chained index",
